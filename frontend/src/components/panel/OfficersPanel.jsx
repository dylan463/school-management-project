    ...
    try {
      if (isEdit) {
        ...
        await update.mutateAsync(
          ...
          {
            onSuccess: () => {
              toast.success("Officier modifié avec succès");
              onSuccess?.();
            },
          }
        );
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 p-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          placeholder="Ex: officier@ecole.com"
        />
        {getError("email") && (
...
function DeleteConfirm({ Data, onSuccess }) {
  const destroy = useDeleteOfficer();
  const [loading, setLoading] = useState(false);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success("Officier supprimé avec succès");
      onSuccess?.();
    } catch (error) {
      handleErrors(error);
  ...
  return (
    <div className="p-3">
      <p>
        Voulez-vous vraiment supprimer l'Officier {Data.first_name} {Data.last_name} ?
      </p>
...
export default function OfficiersPanel() {
  ...
  const { data, isLoading } = useOfficers(filters);
