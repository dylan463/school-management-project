...
function ProgressRow({ status, progress }) {
    ...
    return (
        <div className={`rounded px-3 h-[35px] mb-2 flex items-center justify-between w-full gap-3 ${bgColor}`}>
...
function ResultBadges({ result }) {
    ...
    return (
        <div className="mb-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 w-full">
            <div className="flex gap-4 text-xs">
                <a href={result.report_file} className="bg-green-600 text-white font-bold flex items-center px-2 justify-center w-full h-[35px] rounded-lg">Télécharcher le Rapport</a>
            </div>
        </div>
    )
}
...
const ReenrollmentUploadPanel = () => {
    ...
    const { data: jobs, isLoading: isJobsLoading } = useImportJobs({ import_type: "ENROLLMENT", status: "LOADING" })
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { data } = await api.post("/portal/enrollments/upload/", formData, {
                headers: { "Content-Type": "multipart/form-data" },
            })
            queryClient.invalidateQueries({
                queryKey: ['importJobs', { import_type: "ENROLLMENT", status: "LOADING" }],
            })
        } catch (err) {
    ...
    return (
        <Card className="mb-6">
            <div className="flex m-2">
                {/* Description */}
                <div className="">
                    <p className="font-body-md text-body-md text-secondary w-[300px]">
                ...
                <div className="h-full flex flex-col justify-end mx-3 mt-2">
                    ...
                    ) : <div className="border-2 border-dashed rounded-xl mx-auto w-[200px] h-[200px] border-black bg-white">
                    </div>}
                ...
                {(fileError || progress || result) && <div className="flex flex-col items-center p-4 h-full justify-center gap-2 w-[300px]">
                    {/* Erreurs */}
                    {(fileError) && (
...
export default ReenrollmentUploadPanel