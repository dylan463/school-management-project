...
function AddOrEditForm({ initialData = {}, onSuccess }) {
  ...
  const { data: schoolyearsData } = useSchoolyears(isEdit ? null : { page_size: 100 });
  const { data: formationsData } = useFormations(isEdit ? null : { page_size: 100 });
  const { data: semestersData } = useSemesters(isEdit ? null : { page_size: 100 });

  const schoolyears = schoolyearsData?.results || [];
